...
var GetAllSurveysByTenant = func(w http.ResponseWriter, r *http.Request) {
	...
	surveyNameFilter := r.URL.Query().Get("survey_name")
	...
	data, err := models.GetAllSurveysFromDB(tenantID, page, pageSize, statusFilter, accountNameFilter, userID, uint(surveyFormatIDFilter), surveyNameFilter)
	if err != nil {
		http.Error(w, constants.UPDATED_FAILED, http.StatusInternalServerError)
		return
	}
